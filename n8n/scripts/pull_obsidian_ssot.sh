...
for pair in prompt_pairs:
    ...
    write_text(Path('local-files/_managed/prompts') / file_name, fetch_remote_text(f"{prompts_dir}/{file_name}"))
...
for pair in local_context_pairs:
    _, file_name = pair.split(':', 1)
    write_text(Path('local-files/_managed/context/workflow') / file_name, fetch_remote_text(f"{local_context_dir}/{file_name}"))

for pair in config_pairs:
    _, file_name = pair.split(':', 1)
    write_json(Path('local-files/_managed/config') / file_name, fetch_remote_text(f"{config_dir}/{file_name}"))

for pair in schema_pairs:
    _, file_name = pair.split(':', 1)
    write_json(Path('local-files/_managed/schemas') / file_name, fetch_remote_text(f"{schema_dir}/{file_name}"))
PY
